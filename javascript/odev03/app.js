...
const btnItems = document.querySelector(".btn-container");
...
btnItems.innerHTML = `
    ...
    <button class="btn-item">China</button>
`;
btnItems.addEventListener("click", x => {
  if (x.target.tagName == "BUTTON") {
    if (x.target.innerText == "All") {
      allShowMenu();
    } else {
      showMenu(x.target.innerText);
    }
  }
});
function allShowMenu() {
  menuItems.innerHTML = "";
  menu.forEach(item => {
    menuItems.innerHTML += `
    <div class="menu-items col-lg-6 col-sm-12"> 
      ...
      <div class="menu-info">
        ...
        <div class="menu-category">Country: <span>${item.category}</span></div>
      </div>
    </div>
    `;
  });
...
function showMenu(cat) {
  if (cat) {
    ...
    menuItems.innerHTML = catMenu.join("");
  } else {
    allShowMenu();
  }
}
